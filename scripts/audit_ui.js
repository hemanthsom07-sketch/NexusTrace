...
async function runAudit() {
  ...
  try {
    ...
    const consoleLogs = await page.$$eval('.data-table-card div', (els) => {
      const logDiv = Array.from(els).find((e) => e.innerText && e.innerText.includes('[') && e.innerText.includes('Normalizing'))
      return logDiv ? logDiv.innerText : 'NO_LOGS'
    })
    console.log('Pipeline Console Output Sample:\n', consoleLogs.split('\n').slice(-5).join('\n'))
